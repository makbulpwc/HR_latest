...
def readanyotherformat(filename,s):
   ...
   if formatfromupload == "XLSX" or formatfromupload == "xlsx":
      ...
      for index, row in df.iterrows(): 
         ...
         cursor.execute("INSERT INTO fresh_candidate_upload (fname,lname,phone_no,upload_time,current_stage,new_stage) VALUES (%s, %s,%s,%s,%s,%s)", (row['FirstName'],row['LastName'],row['Phone No'],'NOW()','Pending','Pending'))         
...
def enterdetails():  
   ...
   if request.method == 'POST':
      ...
      if(rcurrstatus == "rejected"  or rnewstatus == "rejected"):
         Final_Status = "Closed"
      elif(rcurrstatus == "selected"  and  rnewstatus == "rejected"):
         Final_Status = "Closed"
      elif Final_Status != "Closed":
         Final_Status = "Yet to Process"

      elif(selectcstatus == "Technical Interview" and rcurrstatus == "selected"):
      ...
      elif(selectcstatus == "offer decline" and rcurrstatus == "selected"):
         selectnstatus = "Closed";
         Final_Status = "Closed"

      elif(selectcstatus == "workday approval" and rcurrstatus == "selected"):
         selectnstatus = "Yet to Process";   
         Final_Status = "Yet to Process"
...
def upload_report():
   ...
   if(dept_name == "all"):
      cursor.execute("select fname,final_status,lname,phoneno,i_date,updated_time  from (select fname,final_status, lname,new_stage,phoneno,i_date,updated_time, row_number() over (partition by fname order by i_date desc) as r1 from candidate_stages)a where r1 =1  and final_status <> 'Closed'")
      mobile_records = cursor.fetchall()
   else:
      cursor.execute("select fname,lname,phoneno,i_date,updated_time  from (select fname, final_status,lname,new_stage,phoneno,i_date,updated_time, row_number() over (partition by fname order by i_date desc) as r1 from candidate_stages)a where r1 =1 and  a.final_status <> 'Closed' and new_stage= %s",[dept_name])
      mobile_records = cursor.fetchall()


   cursor.execute("select new_stage,count(r1) from (select new_stage,final_status, row_number() over (partition by fname order by i_date desc) as r1 from candidate_stages)a where r1 =1 and final_status <> 'Closed' group by 1")
...
@app.route('/closedreport', methods = ['GET', 'POST'])
def closed_report():

   cursor.execute("select fname,lname,phoneno,i_date,new_stage from candidate_stages where final_status='Closed'")
   mobile_records = cursor.fetchall()

   return render_template('closed_report.html',closeddata =mobile_records)  
